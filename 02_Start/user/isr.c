...
#include "../code/motor_driver/motor_encoder.h"
//#include "../code/imu_driver/imu_driver.h"  // IMU已禁用

// 调试计数器定义（ISR 中递增）
volatile uint32 g_vsync1_count = 0;
volatile uint32 g_vsync2_count = 0;
volatile uint32 g_dma1_count = 0;
volatile uint32 g_dma2_count = 0;
volatile uint32 g_frame_done_count = 0;
volatile uint32 g_frame_process_count = 0;
volatile uint32 g_main_loop_count = 0;
...
IFX_INTERRUPT(cc60_pit_ch0_isr, 0, CCU6_0_CH0_ISR_PRIORITY)
{
    ...
    // IMU660RA数据采集与姿态更新 (5ms周期) — 已禁用
    // imu_driver_isr_update();
}
...
IFX_INTERRUPT(cc60_pit_ch1_isr, 0, CCU6_0_CH1_ISR_PRIORITY)
{
    ...
    // 每10ms读取编码器 (200MHz / 2000000 = 10ms)
    encoder_isr_update();
}
